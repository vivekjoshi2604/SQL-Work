...
select order_id,customer_name,customer_id,amount, row_number() over(order by amount desc) from orders;

select category,order_id, amount, rank() over(partition by category order by amount desc) from orders;

select order_id ,category, amount,avg(amount) over(partition by category ),
round(amount-avg(amount) over(partition by category)) from orders;

select * from
(select customer_id,customer_name, order_id ,amount, max(amount) over(partition by customer_id) as customer_max from orders) as t
where amount=customer_max;

select * from
(select customer_id,customer_name, order_id ,amount,rank() over(partition by customer_id order by amount desc) as customer_rank from orders) as t
...
select customer_id,customer_name,order_id ,amount, sum(amount) over(partition by customer_id),
round(amount/sum(amount) over(partition by customer_id)*100,2) from orders ;
